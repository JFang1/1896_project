...
*/
/**************************************************************************/
bool Adafruit_BNO055::getEventAcc(sensors_event_t *event)
{
  /* Clear the event */
  memset(event, 0, sizeof(sensors_event_t));
  ...
  /* Get a Euler angle sample for orientation */
  imu::Vector<3> acc = getVector(Adafruit_BNO055::VECTOR_LINEARACCEL);
  event->acceleration.x = acc.x();
  event->acceleration.y = acc.y();
  event->acceleration.z = acc.z();
